...
void WriteResults( const char* file_names[] )
{
	...
	if (out.is_open())
	{
		...
		{
			tmp_box = temp.at(i); //вектор, содержащий box - ы, являющиеся частью рабочего пространства
	...
	if (out.is_open())
	{
		for (int i = 0; i < temp.size(); i++)
		{
			tmp_box = temp.at(i);// вектор, содержащий box-ы, не являющиеся частью рабочего пространства
			tmp_box.GetParameters(x_min, y_min, width, height);
			out << x_min << " " << y_min << " " << width << " " << height << std::endl;
		}
	}
	...
	if (out.is_open())
	{
		for (int i = 0; i < temp.size(); i++)
		{
			tmp_box = temp.at(i);// вектор, содержащий box-ы, находящиеся на границе между "рабочим" и "нерабочим" пространством
			tmp_box.GetParameters(x_min, y_min, width, height);
			out << x_min << " " << y_min << " " << width << " " << height << std::endl;
		}
	}
